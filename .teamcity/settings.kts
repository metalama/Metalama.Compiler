...
object PublicDeployment : BuildType({
    ...
    steps {
        ...
        powerShell {
            ...
            noProfile = false
            scriptArgs = "-Script Build.ps1 -ImageName metalamacompiler-2026.1 -NoBuildImage -Label %system.teamcity.buildType.id%_%build.number% publish --configuration Public --timeout %Publish.Timeout% %Publish.Arguments%"
        }
        powerShell {
            name = "Cleanup Docker containers"
            id = "DockerCleanup"
...
object UpstreamMerge : BuildType({
    ...
    steps {
        powerShell {
            ...
            noProfile = false
            scriptArgs = "-BuildImage -ImageName metalamacompiler-2026.1 -Dockerfile eng-Metalama/docker/Dockerfile.claude "
        }
        powerShell {
            name = "Merge upstream"
            id = "UpstreamMerge"
            ...
            noProfile = false
            scriptArgs = "-Script Build.ps1 -ImageName metalamacompiler-2026.1 -Dockerfile eng-Metalama/docker/Dockerfile.claude -NoBuildImage -Label %system.teamcity.buildType.id%_%build.number% -Snapshot upstream-merge --timeout %UpstreamMerge.Timeout% %UpstreamMerge.Arguments%"
        }
        powerShell {
            name = "Cleanup Docker containers"
            id = "DockerCleanup"
