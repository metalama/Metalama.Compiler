...
| [PlatformSupport.md](docs-Metalama/PlatformSupport.md) | Deciding which target frameworks, Roslyn versions or Visual Studio versions a release ships for, or dropping one. Points at the product-wide doctrine in `metalama/Metalama`, which defines the support rules, names the resulting set (`PB-<release>`) and records the current baseline, and summarises what that baseline constrains here. Read it *before* changing `eng/targets/TargetFrameworks.props`. |
