...
RUN $process = Start-Process C:\vs_buildtools.exe -NoNewWindow -Wait -PassThru `
        -ArgumentList "modify", "--quiet", "--wait", "--norestart", "--nocache", "--installPath", "C:\BuildTools", "--channelId", "VisualStudio.18.Release", "--productId", "Microsoft.VisualStudio.Product.BuildTools", "--add", "Microsoft.Net.Component.4.7.2.SDK", "--add", "Microsoft.Net.Component.4.7.2.TargetingPack", "--add", "Microsoft.NetCore.Component.SDK"; `
    if ($process.ExitCode -ne 0) { `
        Get-ChildItem "$env:TEMP\dd_*.log" -ErrorAction SilentlyContinue | ForEach-Object { `
            Write-Host "=== Contents of $($_.Name) ==="; `
            Get-Content $_.FullName; `
